...
class DashboardActions:
    ...
    def login_and_wait_for_dashboard(self, base_url: str, username: str, password: str) -> None:
        ...
        self.dashboard_page.wait_for_dashboard_loaded(timeout_ms=40000)

    def verify_dashboard_loaded(self) -> None:
        """Verify dashboard is loaded."""
        expect(
            self.dashboard_page.page.locator(self.dashboard_page.DASHBOARD_HEADER).first
        ).to_be_visible()
        expect(
            self.dashboard_page.page.locator(self.dashboard_page.ADMIN_LINK).first
        ).to_be_visible()
        expect(
            self.dashboard_page.page.locator(self.dashboard_page.MY_ACTIONS).first
        ).to_be_visible()
